...
Additionally, we extract all used phonetic symbols and assign integer IDs to them in order to store transcriptions as integer lists that the Needleman–Wunsch algorithm can work on. For our examples, we obtain \autoref{tab:phonetic-encoding}. Note that we consider \textipa{/dZ/} as one symbol, even though it is a combination of \textipa{/d/} and \textipa{/Z/}. The same applies to \textipa{/tS/}. This is to account for the different pronunciation of the combined symbols compared to the individual ones.
